...
# note: to run this code, create a folder in the same directory of the code
# titled "Simulation ID" with the data file inside in the form "dataID" 

ID = 4

folder = "Simulation {}/".format(ID)
dirname = folder + "Split" # directory with split files
...
if not os.path.exists(folder):
    print("No simulation directory found for given ID = {} - please create one".format(ID))

if SPLIT:

    # read all lines of the file into list
    lines = list()
    with open(folder + "data{}.csv".format(ID)) as file:
        lines = file.readlines()
       
    # note: to split the panel data into individual files we need to account
    # for the fact that they might appear in different orders in the full data files
     
    # make dictionary of panel names and indexes
    panels = {
        "Panel+Y" : 0,
        "Panel-Y" : 0,
        "Panel+X" : 0,
        "Panel-X" : 0,
        "All Solar Panel Groups" : 0
    }
    ...
    for line in lines:
        ...
        if stripped == "\"Panel+Y\"":
            ...
            panels["All Solar Panel Groups"] = i
        i += 1
    
    # sort panel names by index (in order of appearance)
    sorted_panels = dict(sorted(panels.items(), key = lambda x:x[1])) 
        
    if not os.path.exists(dirname):
        os.mkdir(dirname)
        
    panel_keys = list(sorted_panels.keys())
    
    for i in range(len(panel_keys) - 1):
        
        # write individual panel data files
        with open(dirname + "/{}.csv".format(panel_keys[i].lower()), "w") as file:
            start = sorted_panels[panel_keys[i]]
            end = sorted_panels[panel_keys[i+1]]
            file.writelines(lines[start + 2 : end - 1])

    # write all panels data file
    with open(dirname + "/all.csv", "w") as file:
        file.writelines(lines[panels["All Solar Panel Groups"] + 2 : -2])
...
monthSunPhaseDurationMedians = list()
monthSunPhaseDurations = list()
...
for i in range(len(keys)):
    ...
    monthSunPhaseDurationMedians.append(statistics.median(thisMonthDurations))
    monthSunPhaseDurations.append(thisMonthDurations)
...
print("Range of sun phase duration medians over one month:", "%.3f" % min(monthSunPhaseDurationMedians) + "-" + "%.3f" % max(monthSunPhaseDurationMedians), "min")
...
plt.savefig(folder + "Plots/avgyear.png", dpi=300)
...
plt.savefig(folder + "Plots/sunphaseavgs.png", dpi=300)
...
plt.savefig(folder + "Plots/sunphasedurations.png", dpi=300)
...
plt.savefig(folder + "Plots/sunphasedurationsyear.png", dpi=300)

# plotting medians for sun phase duration over one month
col = ['C{}'.format(x) for x in range(12)]
fig, ax = plt.subplots(figsize=(12,8))
plt.grid()
plt.bar(keys, monthSunPhaseDurationMedians, color=col)
ax.set_ylabel("Median Duration of Sun Phases for each Month (min)")
ax.set_xlabel("Month")
plt.title("Median Duration of Sun Phases Over Year")
plt.savefig(folder + "Plots/sunphasemediansyear.png", dpi=300)
...
plt.savefig(folder + "Plots/sunphasehistogram.png", dpi=300)

# plotting power graphs for each month
if MONTH_PLOTS:
    print("Plotting month graphs...")
    if not os.path.exists(folder + "Plots/Months"):
        os.mkdir(folder + "Plots/Months")
    ...
    for i in range(len(keys)):
        if not i == 11:
            ...
            plt.savefig(folder + "Plots/Months/{}.png".format(thisMonth), dpi=300)
        else:
            thisMonth = keys[i]
            fig, ax = plt.subplots(figsize=(16,8))
            plt.grid()
...
if MONTH_PHASE_PLOTS:
    ...
    if not os.path.exists("Plots/Month Phases"):
        os.mkdir(folder + "Plots/Month Phases")
    ...
    for i in range(len(keys)):
        ...
        for line in leg.get_lines():
            line.set_linewidth(4.0)
        plt.savefig(folder + "Plots/Month Phases/{}.png".format(thisMonth), dpi=300)
