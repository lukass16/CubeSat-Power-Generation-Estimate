...
# note: this is a roughly adapted version of the analysis.py code, to run it place the data file
# with the name in the form "dataID" in the same folder as the code and set the corresponding ID
...
monthCount = 3
dt = 60 # time between data points

# flags
SPLIT = True
MONTH_PLOTS = False
MONTH_PHASE_PLOTS = False
...
if SPLIT:
    ...
    with open(dirname + "/panel+Y.csv", "w") as file:
        file.writelines(lines[panelPosYStart + 2 : allPanelStart-1])
    ...
    with open(dirname + "/panel-Y.csv", "w") as file:
        file.writelines(lines[panelNegYStart + 2 : panelPosYStart-1])
...
# calculate average solar power generation over time period
...
keys = list(months.keys())[:monthCount]
monthAverages = list()

for i in range(len(keys)):
    if not i == monthCount - 1:
...
print("Amount of sun phases during period:", len(sunPhaseAverages))
